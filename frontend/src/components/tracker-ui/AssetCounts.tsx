  ...
  return (
    ...
    >
      <div className='flex flex-col bg-secondary rounded-lg border px-4 py-2 w-[125px]'>
        <span className='text-xs font-regular sm:font-bold text-accent-foreground self-end overflow-ellipsis whitespace-nowrap'>TOTAL ASSETS</span>
      ...
      <div className='hidden sm:block'>
        {Object.entries(assetCounts).map(([category, count], index) => (
          ...
          >
            <span className='text-xs font-regular sm:font-bold text-muted-foreground text-end text-ellipsis overflow-hidden whitespace-nowrap w-full'>{category.toUpperCase()}</span>
            <span className='text-xl sm:text-3xl font-bold text-accent-foreground self-end'>
