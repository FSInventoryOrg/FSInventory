...
import { ChevronLeftIcon, ChevronsUpDownIcon, PencilIcon } from 'lucide-react';
...
const Options = ({ property, colorSelect=false, tagSelect=false, field, className }: OptionsProps) => {
  ...
  const { mutate: updateAssetProperty, isPending: isAssetEditPending } = useMutation({
    mutationFn: imsService.updateAssetProperty,
  });
            ...
            <Button 
              ...
              onClick={() => {
                if (newOption && optionValues && optionToEdit) {
                  ...
                  updateAssetProperty({ property: newOption.property, value: optionToEdit, newValue: typeof newOption.value === 'object' ? newOption.value.value : newOption.value });
