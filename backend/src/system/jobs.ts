...
export const setDefaults = async () => {
  ...
  const newCreds: any = {
    ...
    clientID: process.env.CATALYST_SELF_CLIENT_ID,
    clientSecret: process.env.CATALYST_SELF_CLIENT_SECRET,
...
export const removeStatus = async () => {
  ...
  const options: any = await Option.aggregate().match({});
  ...
  if (options?.length > 0) {
    ...
    if (option.status) {
      ...
      option.status = option.status.filter(
        (f: any) => !statusToDelete.includes(f["value"])
      );
