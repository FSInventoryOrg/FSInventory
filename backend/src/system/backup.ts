...
import {
  ...
  getDirPath,
  getFileFromDir,
  readJSONDataFromCSVFile,
  readJSONDataFromExcelFile,
  readJSONDataFromJSONFile,
  saveFileIntoDir,
  saveFileIntoDirFromBase64,
...
export const extractDocuments = async (fileFormat: string = "json") => {
  ...
  for (const collection of collections) {
    ...
    if (fileFormat === "json") {
      ...
      const sanitizedDocuments = documents.map((doc) =>
        convertObjectIdToId(doc)
      );
      // Stringify nested fields
      const stringifiedDocuments = sanitizedDocuments.map((doc) =>
        stringifyNestedFields(doc)
      );
      // Format dates
      const formattedDocuments = stringifiedDocuments.map((doc) =>
      ...
      await saveFileIntoDir(
        collectionsDir,
        `${collection}.${fileFormat === "csv" ? "csv" : "xlsx"}`,
        buffer,
        true
      );
    }
  ...
  const pathFile = await saveFileIntoDir(
    archiveDir,
    zipFilename,
    zip.toBuffer(),
    true
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (initiateBackup["status"] === 404)
        return res.status(404).json({ message: "No collections found" });
      if (initiateBackup["status"] === 500)
      ...
      res
        ...
        .setHeader(
          "Content-Disposition",
          `attachment; filename=${initiateBackup.zipFilename}`
        )
        .end(filesrc);
    } catch (error) {
      console.log(error);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (uploadData.status !== 200)
        return { status: uploadData.status, message: uploadData.message };
      const srcFormat = uploadData.data;
      // EXTRACTION PROCESS
      // delete previous extracts
      await deleteAllFilesInDir("/public/import/collections");
      // save file
      const backupDir = await saveFileIntoDirFromBase64(
        "/public/import/archive",
        "backup.zip",
        srcFormat?.base64
      );
      ...
      const fileFormat = getUploadFormat("/public/import/collections")?.split(
        "."
      )[1];
      if (!fileFormat) {
      ...
      const staleCollections: {
        [key: string]: { current: any[]; backup: any[] };
      } = {};
      let hasStale = false;
      ...
      for (const collection of collectionList) {
        ...
        try {
          let backupJsonData;
          if (fileFormat === "json") {
            backupJsonData = await readJSONDataFromJSONFile(
              "/public/import/collections",
              `${collection}.${fileFormat}`
            );
          } else if (fileFormat === "xlsx") {
            backupJsonData = await readJSONDataFromExcelFile(
              "/public/import/collections",
              `${collection}.${fileFormat}`
            );
          } else {
            backupJsonData = await readJSONDataFromCSVFile(
              "/public/import/collections",
              `${collection}.${fileFormat}`
            );
          }
          if (!backupJsonData) continue;
          const backupDeserializedData =
            backupJsonData.map(convertIdToObjectId);
          // Create a map of backup data for quick lookup
          const backupDataMap = new Map<string, any>(
            backupDeserializedData.map((doc: any) => [doc._id.toString(), doc])
          );
          ...
          for (const doc of collectionDocuments) {
            ...
            if (backupDoc) {
              ...
              const backupUpdatedAt = backupDoc.updated
                ? new Date(backupDoc.updated)
                : null;
              // Check if the document is stale
              if (
                dbUpdatedAt &&
                backupUpdatedAt &&
                dbUpdatedAt > backupUpdatedAt
              ) {
                ...
                const backupCreatedAt = backupDoc.created
                  ? new Date(backupDoc.created)
                  : null;
                // Check by created at
                if (
                  dbCreatedAt &&
                  backupCreatedAt &&
                  dbCreatedAt > backupCreatedAt
                ) {
                  currentStaleDocs.push(doc);
                  backupStaleDocs.push(backupDoc);
                  hasStale = true;
          ...
          console.error(
            `Error processing file for collection ${collection}:`,
            errorFile
          );
        }
      }
    ...
    try {
      ...
      for (const collection of collectionList) {
        try {
          let importJsonData;
          if (fileFormat === "json") {
            importJsonData = await readJSONDataFromJSONFile(
              "/public/import/collections",
              `${collection}.${fileFormat}`
            );
          } else if (fileFormat === "xlsx") {
            importJsonData = await readJSONDataFromExcelFile(
              "/public/import/collections",
              `${collection}.${fileFormat}`
            );
          } else {
            importJsonData = await readJSONDataFromCSVFile(
              "/public/import/collections",
              `${collection}.${fileFormat}`
            );
          }
          if (!importJsonData) continue;
          const importDeserializedData =
            importJsonData.map(convertIdToObjectId);
          // Loop selected backup data
          if (req.body[collection]) {
            for (const bkpData of req.body[collection]) {
              const bkpConvertedData = convertIdToObjectId(bkpData);
              const bkpIndex = importDeserializedData.findIndex(
                (bkpItem: any) => bkpItem._id.equals(bkpConvertedData._id)
              );
              if (bkpIndex !== -1) {
