...
import { getAssetIndexes } from "../utils/asset-counter";
...
router.post(
  ...
  [
    ...
    check("prefixCode")
      .optional()
      .isString()
      .withMessage("Prefix Code must be a string"),
    check("threshold")
      .optional()
      .isNumeric()
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (data["prefixCode"])
        data["prefixCode"] = data["prefixCode"].toUpperCase().trim();
      data["totalCount"] = 0;
      data["status"] = "";
...
router.put(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const findSameCounter = existingAssetCounter.find((f) => {
        return (
          f["prefixCode"] === data["prefixCode"] &&
          f["_id"].toString() !== data["_id"]
        );
      });
      if (findSameCounter)
        return res.status(400).json({ message: "Asset code already exists" });

      const findSameCatType = existingAssetCounter.find((f) => {
        return (
