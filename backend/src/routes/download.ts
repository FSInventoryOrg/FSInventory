...
router.get('/info/:id', verifyToken, async(req: Request, res: Response) => { 
    try {
...
router.get('/file/:id', verifyToken, async(req: Request, res: Response) => { 
    try {
        const fileID: string = req.params.id;
        ...
        res.status(200)
            ...
            .setHeader('Content-Disposition', `attachment; filename=${attachment['originalName']}`)
            .end(filesrc);
    } catch (error) {
